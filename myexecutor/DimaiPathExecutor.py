...
class DimaiPath(BasePath):
    def __init__(self, name, country, positions: List[DimaiPoint], anchor_name=None, enable=None):
...
class DimaiPathExecutor(BasePathExecutor):
    ...
    @staticmethod
    def load_basepath_from_json_file(json_file_path) -> DimaiPath:
        with open(json_file_path, encoding="utf-8") as r:
            json_dict = json.load(r)
            points: List[DimaiPoint] = []
            for point in json_dict.get('positions', []):
                p = DimaiPoint(x=point.get('x'),
                          y=point.get('y'),
    ...
    def get_closet_dimai(map_controller):
        ...
        missions_screen_points = DimaiPathExecutor.get_mission_template_matched_screen_position()
        # 计算得到世界坐标
        ...
        for mission_world_point in mission_world_points:
            closest:str = DimaiPathExecutor.get_specify_point_closest_mission_json(mission_world_point)
    ...
    def on_move_after(self, point: DimaiPoint):
        if point.type == DimaiPoint.TYPE_TARGET:
            if point.action == DimaiPoint.ACTION_FIGHT:
                self.crazy_f()
                # 开启地脉
                # OCR检测文字：点击浓缩树脂
                # 战斗
            ...
            elif point.action == DimaiPoint.ACTION_REWARD:
...
if __name__ == '__main__':
    ...
    # t.setDaemon(True)
    # t.start()
    DimaiPathExecutor.execute_all_mission()
